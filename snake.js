...
let Snake = function () {
	...
	if (
		...
		alert(
			'#game-area, #start-game, #stop-game, #score id elements are required!'
		);
		return;
	}

	snake.startButton.addEventListener(
		'click',
		function () {
			snake.startGame();
		},
	...
	snake.stopButton.addEventListener(
		'click',
		function () {
			snake.stopGame();
		},
...
Snake.prototype.startGame = async function () {
	...
	if (devices.length !== 0) {
		...
		if (navigator.hid) {
			try {
				...
				console.log(e);
				alert('You need a DualShock 5 Contorller');
				return;
			}
		}
	}
	...
	if (snake.device) {
		await controllerOutput(snake.device, false, 0xff, 0xff, 0xff);
		snake.device.oninputreport = async (e) => {
			const { data, reportId } = e;
			if (reportId === 0x01) {
				snake.controlls.axes.leftStickX = normalizeThumbStickAxis(
					data.getUint8(0)
				);
				snake.controlls.axes.rightStickY = normalizeThumbStickAxis(
					data.getUint8(3)
				);
			}
 ...
 * Ends the game
 */
Snake.prototype.endGame = async function () {
	// alert('Game Over!! Yout score is ' + this.score);
	this.stopGame();
	await controllerOutput(this.device, true, 0xff, 0x00, 0x00);
};
...
Snake.prototype.move = function () {
	...
	let cellCoordinates =
		"[data-x='" + lastSnakeCell.x + "'][data-y='" + lastSnakeCell.y + "']";
	let newSnakeCell = {};
	...
	if (newSnakeCell.x == snake.bait.x && newSnakeCell.y == snake.bait.y) {
		...
		cellCoordinates =
			"[data-x='" + newSnakeCell.x + "'][data-y='" + newSnakeCell.y + "']";
		snakeCellElement = document.querySelector(cellCoordinates);
...
const controllerOutput = async (device, rumble, r, g, b) => {
	const report = new Uint8Array(48);

	// set report ID
	report[0] = 0x02;

	// enable rumble and lightbar
	report[1] = 0xff;

	report[2] = 0xf7;

	//set rumble
	report[4] = rumble && 0x01;
